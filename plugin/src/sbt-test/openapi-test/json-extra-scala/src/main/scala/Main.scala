...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val firstDiscriminator: DiscriminatorObjectDto = DiscriminatorObjectDto.FirstDiscriminatorSubObject(uniqueString = Some("v1"), sharedString = Some("v2"), anotherSharedBoolean = true)
    val secondDiscriminator: DiscriminatorObjectDto = DiscriminatorObjectDto.SecondDiscriminatorObject(uniqueInteger = 2, otherUniqueBoolean = None, sharedString = Some("v4"), anotherSharedBoolean = true)
    testJson(firstDiscriminator)
    testJson(secondDiscriminator)
    val catDiscriminator: PetDto = PetDto.Cat("claw", "Sprinkles")
    val dogDiscriminator: PetDto = PetDto.Dog(10, "Fido")
