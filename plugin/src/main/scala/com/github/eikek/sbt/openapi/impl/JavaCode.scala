  ...
  def defaultTypeMapping(cm: CustomMapping): TypeMapping = {
    case Type.Sequence(param) =>
      defaultTypeMapping(cm)(param).map(el =>
        cm.changeType(
          TypeDef(s"List<${boxed(el).name}>", el.imports ++ Imports("java.util.List"))
        )
      )
    case Type.Map(key, value) =>
      for {
        ...
        TypeDef(
          s"Map<${boxed(k).name},${boxed(v).name}>",
          k.imports ++ v.imports ++ Imports("java.util.Map")
        )
      )
    case Type.Ref(name) =>
  ...
  object BuilderClass {
    ...
    def make(cfg: JavaConfig): PartConv[SourceFile] =
      builderMethod.map(_.newline) ++
        ...
        constant("public static final class Builder") ~ DataClass.parents
          .contramap[SourceFile](_ => cfg.builderParents) ~ constant("{") ++
        DataClass
          .fieldDeclaration("private")
          .map(_.indent(2).newline)
          .contramap(_.fields) ++
    ...
    def setter: PartConv[Field] =
      doc.contramap[Field](_.prop.doc) ++
        constant("public Builder set") + fieldName.map(_.capitalize) + constant(
          "("
        ) + DataClass.fieldType ~
        constant("value) {") ++ constant("this.").map(_.indent(2)) + fieldName ~ constant(
          " = value;"
        ) ++
  ...
  object DataClass {
    def apply(tm: TypeMapping, cfg: JavaConfig): PartConv[SourceFile] =
      forList(annotation, _ ++ _).contramap[SourceFile](_.annot) ++
        constant("public final class") ~ sourceName ~ parents.contramap[SourceFile](
          _.parents
        ) ~ constant("{") ++
        fieldDeclaration("private final")
          .map(_.indent(2).newline)
          .contramap[SourceFile](_.fields) ++
    ...
    def fieldDeclaration(modifier: String): PartConv[List[Field]] = {
      val pc: PartConv[Field] =
        forList(annotation, _ ++ _).contramap[Field](_.annot) ++
          constant(modifier) ~ fieldType ~ string
            .contramap[Field](_.prop.name)
            .map(_.semicolon)
    ...
    def getter: PartConv[Field] = {
      ...
      cond[Field](
        _.prop.nullable,
        fg ++ opt,
        doc.contramap[Field](_.prop.doc) ++ constant("public") ~ fg
      )
    }
    ...
    def setter: PartConv[SourceFile] = PartConv { sc =>
      def withMethod: PartConv[Field] =
        doc.contramap[Field](_.prop.doc) ++
          constant("public") ~ constant(sc.name) ~ constant("with") + fieldName.map(
            _.capitalize
          ) + constant("(") + fieldType ~ fieldName + constant(") {") ++
          constant("return new").map(_.indent(2)) ~ constant(sc.name) + constant(
            "("
          ) + PartConv.ofPart(fieldNameList.toPart(sc.fields)) + constant(");") ++
          constant("}").map(_.newline)
    ...
    val hashcode: PartConv[SourceFile] =
      constant[SourceFile]("@Override") ++ constant("public int hashCode() {") ++
        constant("return Objects.hash(").map(_.indent(2)) + fieldNameList
          .contramap[SourceFile](_.fields) + constant(");") ++
        constant("}")

    def equals: PartConv[SourceFile] = {
      val header: PartConv[SourceFile] =
        constant[SourceFile]("@Override") ++ constant(
          "public boolean equals(Object other) {"
        ) ++
          constant("if (other == this) {").map(_.indent(2)) ++
          constant("return true;").map(_.indent(4)) ++
          constant("}").map(_.indent(2)) ++
          constant("if (other == null || getClass() != other.getClass()) {").map(
            _.indent(2)
          ) ++
      ...
      (header ++ sourceName.map(_.indent(2)) ~ constant("v = (") + sourceName + constant(
        ") other;"
      ) ++
        constant("return").map(_.indent(2)) ~ forListSep(eq, Part(" && ")).contramap(
          _.fields
        ) + constant(";") ++
        constant("}")).map(_.indent(2))
    }
    ...
    def tostring: PartConv[SourceFile] = {
      ...
      constant[SourceFile]("@Override") ++
        ...
        constant("return String.format(\"").map(_.indent(2)) + sourceName + constant(
          "["
        ) + pc + constant("]\", ") ++
    ...
    def fieldAssignList: PartConv[List[Field]] = {
      val pc: PartConv[Field] = PartConv(f =>
        Part("this.") + Part(f.prop.name) ~ Part("=") ~ Part(f.prop.name).semicolon
      )
      forList(pc, (a, b) => a ++ b)
    }
  }
