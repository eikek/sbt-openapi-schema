...
object ElmCode {
  ...
  def defaultTypeMapping(cm: CustomMapping, pkg: Pkg): TypeMapping = {
    case Type.Sequence(param) =>
      defaultTypeMapping(cm, pkg)(param).map(el =>
  ...
  def fileHeader: PartConv[SourceFile] =
    constant("{-- This file has been generated from an openapi spec. --}").map(
      _.newline
    ) ++
  ...
  def generate(sc: SchemaClass, pkg: Pkg, cfg: ElmConfig): (String, String) = {
    ...
    val conv = fileHeader ++ typeAlias.map(_.newline) ++ defaultValue(pkg).map(
      _.newline
    ) ++ cfg.json.jsonCodec
