...
object CodegenSpec extends SimpleTestSuite {
  ...
  test("Running scala") {
    val config = ScalaConfig.default.
      ...
      addMapping(CustomMapping.forSource({ case s => s.addParents(Superclass("MyTrait", Imports.empty, false)) }))
...
//      addMapping(CustomMapping.forSource({ case s => s.addParents(Superclass("MyFunnyClass", Imports("org.myapp.MyFunnyclass"), false)) })).
//      addBuilderParent(Superclass("MyBuilderHelper", Imports("org.mylib.BuilderHelper"), true))
//
//    println("=========")
//    schema.values.foreach { sc =>
//      println("-------------------------------------------------")
